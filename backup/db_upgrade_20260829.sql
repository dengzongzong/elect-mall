-- ============================================================
-- 时运商城 mall_db 数据库升级脚本 (幂等)
-- 生成时间: 2026-08-29 10:40:27
-- 说明: 1) 确保全部表结构存在(IF NOT EXISTS, 不破坏现有数据)
--       2) 补充缺失的种子配置(按 key 判重)
-- ============================================================
USE `mall_db`;
...
-- 补充缺失的系统配置种子数据
-- ------------------------------------------------------------
INSERT INTO `sys_config` (`config_key`, `config_value`, `created_at`, `updated_at`, `deleted`) SELECT 'site_name', '时运商城', NOW(), NOW(), 0 FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM `sys_config` WHERE `config_key`='site_name');
INSERT INTO `sys_config` (`config_key`, `config_value`, `created_at`, `updated_at`, `deleted`) SELECT 'site_keywords', '电子元器件,商城', NOW(), NOW(), 0 FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM `sys_config` WHERE `config_key`='site_keywords');
INSERT INTO `sys_config` (`config_key`, `config_value`, `created_at`, `updated_at`, `deleted`) SELECT 'site_description', '电子元器件一站式采购平台', NOW(), NOW(), 0 FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM `sys_config` WHERE `config_key`='site_description');

-- 补充缺失的短信通知类型种子数据
INSERT INTO `sms_config` (`type`, `enabled`, `phones`, `created_at`, `updated_at`, `deleted`) SELECT 'order', 0, '', NOW(), NOW(), 0 FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM `sms_config` WHERE `type`='order');
INSERT INTO `sms_config` (`type`, `enabled`, `phones`, `created_at`, `updated_at`, `deleted`) SELECT 'inquiry', 0, '', NOW(), NOW(), 0 FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM `sms_config` WHERE `type`='inquiry');
INSERT INTO `sms_config` (`type`, `enabled`, `phones`, `created_at`, `updated_at`, `deleted`) SELECT 'feedback', 0, '', NOW(), NOW(), 0 FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM `sms_config` WHERE `type`='feedback');
-- 升级完成
